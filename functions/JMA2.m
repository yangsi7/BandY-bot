function jma = JMA(price,varargin)
 A.L=200;
 A.phi=50;
 A.pow=2;
...
%Alpha
 ...
 price = price;
 e0 = nan(size(price)); e0(1)=(1-Alpha)*price(1);
 e1 = nan(size(price)); e1(1)=(price(1)-e0(1))*(1-Beta);
 e2 = nan(size(price)); e2(1)=(e0(1)+phi_r*e1(1));
 e3 = nan(size(price)); e3(1)=e2(1)*(1-Alpha)^2;
 jma = nan(size(price)); jma(1)=e3(1);
