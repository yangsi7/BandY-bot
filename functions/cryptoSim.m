...
A.thrCloseShort=0.4;
A.strategy = 'yhat_fre';
A.StopBuy = [0.015,0.001]; % Need to make adaptative to volatility
A.StopSell = [0.015,0.001];; % Idem
...
for i = idxSigs(1) : idxSigs(end)
   ...
   if buySig(i) & bought.status(i) == 0
      ...
      bought.status(i)=1;
      stopLoss(i) = (1-A.StopSell(1)).*tmw.Close(i);
   end
   ...
   if shortSig(i) & short.status(i) == 0
      ...
      short.status(i)=1;
      limitOrder(i) = (1+A.StopBuy(1)).*tmw.Close(i);
   end
