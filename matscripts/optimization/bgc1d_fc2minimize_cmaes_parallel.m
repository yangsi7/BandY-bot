...
%ParVal = ParMin + ParStart .* ParNorm;
 ...
 parfor indm=1:Nm
   ...
   %tables
   if strat == 2
   [varnames,tmw] = TA.strat1(FunArg.tmw,'Timeindex', ttime,...
      ...
      'macdfast'  ,round(ParVal(8,indm)),...
      'macdsignal',round(ParVal(9,indm)),...
      'macdlong'  ,round(ParVal(10,indm)),...
      'svolper'   ,round(ParVal(11,indm)),...
      'svolf'     ,ParVal(12,indm));

      [gains, drawdown, ~,~] = cryptoSimulation_andyopt(tmw,'strat','Andy2','params',1,...
         'prelstop',ParVal(13,indm),...
         'presstop',ParVal(13,indm),...
         'tp1',ParVal(14,indm),...
         'tp2',ParVal(15,indm),...
         'ftp',ParVal(16,indm),...
         'adxth',ParVal(17,indm),...
         'rsiobos',ParVal(18,indm), ...
         'params',1);
   elseif strat == 1
   [varnames,tmw] = TA.strat1(FunArg.tmw,'Timeindex', ttime,...
      'rfper'     ,round(ParVal(1,indm)),...
      ...
      [gains, drawdown, sp,~] = cryptoSimulation_andyopt(tmw,'strat','Andy','params',4,...
         'reInvest', 1,...
         ...
         'prelstop',ParVal(21,indm),...
         'presstop',ParVal(22,indm),...
...
%      tcost = -sign(gains).*(gains.^2) + (abs(gains).*drawdownmax./100).^2;
      w1=0.5;w2=0.5;
      tcost = w1*tout.cycle.invstdPR.^2 + w2*(drawdown./100).^2;
      tcost(tcost==inf)=2;
      tcost(isnan(tcost))=2;
   else
      tcost = -sign(gains).*(gains.^2) + (drawdown).^5.5
      tcost(tcost==inf)=10000.^2;
      tcost(isnan(tcost))=10000.^2;
   end
%    tcost = -sign(gains).*(gains.^2);
%    if drawdown > 15
%       tcost=10000.^2;
%    end
